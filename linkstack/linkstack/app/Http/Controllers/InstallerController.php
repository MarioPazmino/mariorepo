    ...
    public function mysqlTest(Request $request)
{
    ...
    try {
        Artisan::call('db:seed --class="PageSeeder" --force');
    } catch (Exception $e) {
        $failed = true;
        \Log::error('PageSeeder failed: ' . $e->getMessage());
    }
    ...
    try {
        ...
        $failed = true;
        \Log::error('ButtonSeeder failed: ' . $e->getMessage());
    }

    try {
        $buttonsCount = DB::table('buttons')->count();
    } catch (Exception $e) {
        $failed = true;
        \Log::error('Failed to count buttons: ' . $e->getMessage());
    }

    if ($failed) {
        $this->switchToSQLite();
        return redirect(url('?error'))->with('error', 'MySQL connection failed. Switched to SQLite.');
    } else {
        return redirect(url('?4'));
    }
}
...
{
    if (EnvEditor::keyExists('DB_CONNECTION')) {
        EnvEditor::editKey('DB_CONNECTION', 'sqlite');
    } else {
        EnvEditor::addKey('DB_CONNECTION', 'sqlite');
    }

    if (EnvEditor::keyExists('DB_HOST')) {
        EnvEditor::deleteKey('DB_HOST');
    }

    if (EnvEditor::keyExists('DB_PORT')) {
        EnvEditor::deleteKey('DB_PORT');
    }

    if (EnvEditor::keyExists('DB_DATABASE')) {
        EnvEditor::deleteKey('DB_DATABASE');
    }

    if (EnvEditor::keyExists('DB_USERNAME')) {
        EnvEditor::deleteKey('DB_USERNAME');
    }

    if (EnvEditor::keyExists('DB_PASSWORD')) {
        EnvEditor::deleteKey('DB_PASSWORD');
    }
}
